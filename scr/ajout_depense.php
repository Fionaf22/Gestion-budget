...
$Username='shin';
$sqlCreate = "CREATE TABLE if not exists `gestion_argent`.$Username ( `idDepense` INT NOT NULL AUTO_INCREMENT ,`detail_depense_text` VARCHAR(100) NOT NULL , `date_depense` VARCHAR(15) NOT NULL , `montant_depense_number` DECIMAL(10, 2) NOT NULL , `couleur_depense` VARCHAR(30) NOT NULL , `type_depense` VARCHAR(100) NOT NULL, `optgroup_label` VARCHAR(100) NOT NULL , `note_depense` VARCHAR(250) NOT NULL  ,  PRIMARY KEY (`idDepense`)) ENGINE = InnoDB;";
  ...
  if (isset($_POST['bouton_depense'])) {
    if (!isset($_SESSION['form_submitted'])) {
      if (!empty($montant_depense_number)){
        $sql = "INSERT INTO `gestion_argent`.$Username (couleur_depense, detail_depense_text,montant_depense_number, date_depense,type_depense,optgroup_label, note_depense) VALUES (\"$couleur_depense\" , \"$detail_depense_text\" ,\"$montant_depense_number\", \"$date_depense\",   \"$type_depense\",\"$optgroup_label\", \"$note_depense\" );";
